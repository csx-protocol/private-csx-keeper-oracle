...
export class Web3Service implements OnModuleInit {
  ...
  async listenToLogs(_blockHeight: number) {
    ...
    this.logSubscription = this.walletService.wallet.alchemy.ws.on(filter, async (log, event)=>{
      ...
      if(reciept.confirmations == null) {
        this.logger.warn(`Transaction Failed: txHash: ${txHash}. Topic: ${log.topics[0]} Status: ${reciept.status}. Confirmations: ${reciept.confirmations}`);
        return;
      }
